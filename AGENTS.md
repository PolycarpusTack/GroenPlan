...
- **AI Auto-fill** (`GroenPlan_AutoFill_Prompt_Spec.md`) — Codex Sonnet with prompt caching. Every field carries `FieldWithSource<T>` with `source`, optional `fallback` flag, and `note`. SQLite cache keyed on scientific name (case-insensitive). Targets: ≥90% accuracy, p95 ≤4s, ≈0.4¢/plant.

### AI Integration

| Gebruik | Model | Waarom |
|---|---|---|
| Plant auto-fill | `Codex-sonnet-4-6` + prompt caching | Snelheid + kostprijs; systeem-prompt gecached |
| Tuinontwerp suggesties | `Codex-opus-4-7` | Complexe redenering over begeleidingsplanten |
| Foto-herkenning | PlantNet API → auto-fill confidence | Externe specialist; voedt zekerheidscore |

The Codex API must always be wrapped in an adapter (`src/services/autofill.ts`) — domain code must never import `@anthropic-ai/sdk` directly (guardrail `anti-layer-bleeding` + `G-TS-01`).
